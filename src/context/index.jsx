...
import { useAddress, useContract, useMetamask, metamaskWallet, useContractWrite, useMetadata } from '@thirdweb-dev/react';
import { ethers } from 'ethers';

const StateContext = createContext();

export const StateContextProvider = ({ children }) => {
    const { contract } = useContract("0x5Db77AD3F7E7701F51e6a757aD77e6f4E96Efe6E");
  ...
  const publishCampaign = async(form)=>{
    ...
    const data = await createCampaign({ args: [address, form.title, form.description, form.target, new Date(form.deadline).getTime(), form.image]})
        console.log("Campaign Create Transaction Successful", data);
    }catch(err){
        console.log("Campaign Create Transaction UNsuccessful", err);
    }
  }
